...
class PingRecupSpidTask implements CmsRegularTask
{
   ...
   public function test($time = '')
   {
      ...
      if ( ($time - 60*60 ) >= $last_execute )//toutes les minutes  !!
   ...
   public function execute($time = '')
   {
	...
	$query = "SELECT j.licence FROM ".cms_db_prefix()."module_ping_recup_parties AS rp, ".cms_db_prefix()."module_ping_joueurs AS j  WHERE j.licence = rp.licence AND j.actif = '1' AND maj_spid < NOW()-INTERVAL 3 DAY AND saison = ? ORDER BY maj_spid DESC LIMIT 25 ";
