...
$query="SELECT CONCAT_WS(' ', j.nom, j.prenom) AS joueur, SUM(sp.pointres) AS Total, sp.licence FROM ".cms_db_prefix()."module_ping_parties_spid AS sp, ".cms_db_prefix()."module_ping_joueurs AS j WHERE sp.licence = j.licence AND sp.saison = ? ";	
if($phase == 1)
{
...
if ($dbresult && $dbresult->RecordCount()>0)	
	{
		while($row= $dbresult->FetchRow())
		{
			...
			$onerow->details= $this->CreateLink($id, 'user_results', $returnid, 'Détails',array('licence'=>$row['licence']));
